   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         if ( o.parent_author == STEEMIT_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.head_block_time() + STEEMIT_CASHOUT_WINDOW_SECONDS;
         }
         else
         {
...
{ try {
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         FC_ASSERT( _db.head_block_time() < comment.cashout_time - STEEMIT_UPVOTE_LOCKOUT, "Cannot increase payout within last twelve hours before payout." );
      }
